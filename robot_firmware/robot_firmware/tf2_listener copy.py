...
class TfListener(Node):
    def __init__(self):
        ...
        #self.timer = self.create_timer(1, self.timer_clb)
    ...
    def timer_clb(self):
        ...
        while(distance > 0.1):
            try:
                ...
                #     target_time=rclpy.duration.Duration(seconds=0.0),
                #     source_frame=ref_frame,
                #     source_time=rclpy.duration.Duration(seconds=0.0),
                #     fixed_frame="odom",
                #     timeout=rclpy.duration.Duration(seconds=0.1)
                # )
                #clk = self.get_clock().now()
                #clk.nanoseconds=0
            ...
            #print(self.get_clock().now().to_msg().seconds)
            ...
            if abs(angle) > 0.35: # 20 degree
                msg.linear.x = 0.0
                msg.angular.z = 0.20 * angle / abs(angle)
            else:
                pass
                #msg.linear.x = 0.25
                #msg.angular.z = scale_rotation * angle
...
def main(args=None):
    ...
    rclpy.spin_once(table_detection_node)

    table_detection_node.timer_clb()
